...
As you can see, the method detects one shift near the end of the record. However, one might also want to detect shifts in variance. To do so, all you have to do is specify `cpt.fun = changepoint::cpt.var` (leveraging the [changepoint](https://cran.r-project.org/web/packages/changepoint/index.html) package).
...
Now we see an additional changepoint in record associated with a shift in variance ca. 1450 AD. To check its robustness, we should probably inquire more about the sample depth of the dataset (number of tree cores used to make the composite in each year), as well as the technique used for removing the biological component of the signal. This is a reminder that no technique, however nifty, is a substitute for knowing your data!
